#!/usr/bin/env bash
# pmo-heartbeat-stamp.sh — Lever 1 migration #2: run pmo's heartbeat-stamp WITHOUT an LLM turn.
#
...
#   fire_count at handover: 1289
#
# DISABLING THE ORIGIN CRON: use `cortextos bus update-cron pmo heartbeat-stamp
...
fail() {
  ...
  cortextos bus send-message chief high \
    "wake-on-alert FAILURE — ${AGENT}/heartbeat-stamp (Lever 1 migration #2). Step '${what}' failed: ${detail}. This replaced a daemon cron that fired by LLM injection; the origin cron is DISABLED in pmo/crons.json, so ${AGENT}'s heartbeat is NOT being stamped by anything else. Rollback = re-enable that cron." \
