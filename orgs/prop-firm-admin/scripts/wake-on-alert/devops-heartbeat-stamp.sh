#!/usr/bin/env bash
# devops-heartbeat-stamp.sh — Lever 1 rollout: run devops's heartbeat-stamp WITHOUT an LLM turn.
#
# ORIGIN CRON (disabled, not deleted — rollback is re-enable):
#   agent   devops
#   name    heartbeat-stamp
#   sched   23 * * * *  (hourly at :23)
...
#   fire_count at handover: 2217 (disabled 2026-08-14T13:54Z)
#
# DISABLING THE ORIGIN CRON: use `cortextos bus update-cron devops heartbeat-stamp
...
fail() {
  ...
  cortextos bus send-message chief high \
    "wake-on-alert FAILURE — ${AGENT}/heartbeat-stamp (Lever 1 pilot). Step '${what}' failed: ${detail}. This replaced a daemon cron that fired by LLM injection; the origin cron is DISABLED in devops/crons.json, so ${AGENT}'s heartbeat is NOT being stamped by anything else. Rollback = re-enable that cron." \
