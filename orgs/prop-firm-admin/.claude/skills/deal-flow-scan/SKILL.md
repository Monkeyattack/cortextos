...
- **`query`**: `(to:quietlight@monkeyattack.com OR to:smbdealhunter@monkeyattack.com OR to:acqiireweekly@monkeyattack.com OR to:flippa@monkeyattack.com OR to:dealforce@monkeyattack.com OR to:joinaccresites@monkeyattack.com OR from:rejigg.com OR from:crexi.com) -label:deal-ingested`
...
After vetting, upsert every deal into the deals dashboard so the board stays current. POST to the internal ingest endpoint (server-side, same host, no auth):

```bash
curl -s -X POST http://localhost:3201/api/deals/ingest \
  -H "Content-Type: application/json" \
  -d '{"deals":[ { ...one object per deal... } ]}'
