...
1. **Never fabricate URLs** — listing links must come from the email body or user-supplied input; do NOT guess or construct broker URLs. **BUT: a tracking/redirect link IS a valid body link — use it, don't null it.** Flippa newsletters never expose clean `flippa.com/listings/X` URLs; they only carry obfuscated referral-tracking redirects (`l.flippa.com/...` per-deal "View Business" / "Share with your network" links). These resolve in a browser (with the partner session) and are the only pointer the newsletter provides — so capture the per-deal tracking link as `listing_url`. Leaving Flippa deals' `listing_url` null is wrong (caught 2026-06-12, Chris flagged deal #94). Quiet Light + SMB Deal Hunter DO carry clean direct links — use those as-is. Only "guess/construct" (inventing a URL not in the body) is prohibited. **Dedupe caveat (2026-07-11):** `l.flippa.com` tracking links are unique per email send — the SAME deal gets a DIFFERENT link in every email, so the `listing_url` unique-index will NEVER catch a Flippa re-ingest. Flippa deals therefore dedupe on `(source, name)` only: use the email's **verbatim anchor/headline text** as `name` (no case changes, no added suffixes like "YouTube Channel"), and before boarding, check the board for same-source rows from the past 7 days with similar names (`GET /api/deals?limit=1000`, compare case-insensitively). Also beware **anchor misalignment**: Flippa emails interleave non-deal links (Spotify podcast, free-valuation CTA) between deal blurbs — verify the captured link is the deal's own "View Business" anchor, not an adjacent promo link (caught 2026-07-11: rows 468–474 were a full re-ingest of the Jul-6 email — one row carried the Spotify podcast link as `listing_url` and another deal's figure as `ask`; 6 dup rows deleted).
...
| Wolf of Franchises | workweek@monkeyattack.com — clean `to:` alias, sender `thewolfoffranchises@workweek.com` (Workweek media). **ADDED 2026-07-09 per Chris.** ~2 emails/week (Tue+Thu), subject prefixed 🍟. | "Franchise of the Week" emails — ONE franchise profile per email with FDD-style data: franchise fee, royalty %, brand-fund %, investment range/tiers, unit financial overview, location trends, plus "The Wolf's Take" analysis. **These are franchise OPPORTUNITIES, not for-sale listings — NO ask/SDE → `ask`/`sde`/`multiple` = null (like Rejigg).** Field map: `name` = franchise name; `category` = franchise sector; `location` = HQ (note it's a franchise, buyer picks territory); `notes` ← franchise fee + royalty + investment range + key unit economics + Wolf's take distilled; `body` = the profile prose; `listing_url` = franchise development site link from body (or null); asset_class `main-street`, source key `wolfoffranchises`. Vet on unit economics + category durability, not multiple — default WATCH unless unit economics are standout. Body arrives as raw HTML — strip tags before extraction. | SMB tweet-of-week, Wolf Bites news roundup, sponsor/partner blocks, subscribe CTAs |
| Accredited | joinaccresites@monkeyattack.com — clean `to:` alias (like Quietlight/Flippa). **CONFIRMED LIVE 2026-06-30**: real emails are a beehiiv **"Deal of the Day"** = **ONE deal per email**, sender `accredited@mail.joinaccredited.com` (forwards to the alias). NOT the multi-deal "Daily Digest" we anticipated; and the alias receives Accredited's **general** daily pick — location is NOT TX-filtered (first live deal was Florida), so the "Texas" saved-search is NOT what hits this inbox. **Chris decided 2026-06-30: KEEP-GENERAL — board the general daily Deal of the Day as-is, no TX filter. (closed)** | "Deal of the Day" listing emails (subject `Accredited Deal of the Day - <Day> | <date>`). Source key: `accredited`. **Field map** (per LIVE plain-text email, sample Magnetic Safety Distributor 2026-06-30): the email has a clean labeled block — `name` = the `## <Headline>`; **`ask` ← "Asking Price"**; **`sde` ← "SDE / EBITDA / Cash Flow"** (single combined line — map the value to `sde`, set `ebitda`=null, do NOT duplicate); `revenue` ← "Revenue"; `location` ← "Location"; `category` ← "Business Type"; seller involvement → `notes`; **`multiple` ← "Multiple" given directly — VALIDATE, do NOT recompute** (given 5.03x vs recompute 5.19x = minor, keep as given); **`listing_url` = CLEAN `joinaccredited.com/deals/<slug>` from the `[See Full Analysis]` link — use as-is + as dedupe key**; a secondary `[See Listing]` bizbuysell/broker URL may also appear → put in `notes`; `body` = "About the Business" + "What's our take?" prose. **NO ★ score in the plain-text email** (the 1–5★ rating is web-listing only — don't expect it in email; capture from web only if doing a workup). The Accredited "What's our take?" section gives a ready central-Q + risks — fold into `notes`. **RE caveat still applies:** Accredited asks are often RE-inclusive (land/building/vehicles) — strip RE before judging the operating multiple (e.g. car-wash-with-property); asset-light deals (no owned RE) need no strip. (format corrected 2026-06-30 after first live email) | Account/setup/security (sign-in, password, reset code), Accredited Pro marketing, non-listing content |
| Accredited Pro TX | No alias — hits Chris's MAIN inbox. Sender: `alerts@joinaccredited.com`. **ADDED 2026-07-14**: Texas saved-search alert emails from Accredited Pro (paid tier). ONE deal per email, TX-filtered. | Subject pattern TBD (first live deal: deal #661 CPA firm, 2026-07-14). Source key: `accredited`. **Field map:** same as general Accredited Deal of the Day EXCEPT: (a) **`listing_url` = null** — TX alert plain-text body does NOT include a clean listing URL; note "find via Accredited Pro search" in `notes`; (b) `location` should be Texas or specific TX city; (c) a ★ rating may appear inline (★★★★ seen on deal #661) — capture in `notes`. Dedupe key: `name` (no listing_url available). | Account-setup/security emails from alerts@joinaccredited.com |
...
- **`query`**: `(to:quietlight@monkeyattack.com OR to:smbdealhunter@monkeyattack.com OR to:acqiireweekly@monkeyattack.com OR to:flippa@monkeyattack.com OR to:dealforce@monkeyattack.com OR to:joinaccresites@monkeyattack.com OR to:workweek@monkeyattack.com OR from:rejigg.com OR from:crexi.com OR from:alerts@joinaccredited.com) -label:deal-ingested`
