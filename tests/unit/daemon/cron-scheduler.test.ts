...
/** Pull the UTC components out of an epoch-ms value (matches getUTCHours() in scheduler). */
...
describe('nextFireFromCron', () => {
  ...
  it('wraps to next day when UTC hour 13 has already passed today', () => {
    ...
    expect(loc.hours).toBe(13);
    expect(loc.minutes).toBe(0);
    // Must be tomorrow UTC (date + 1), accounting for month wrap
  ...
  it('handles day-of-week restriction: "0 16 * * 1" — fires on a Monday UTC', () => {
    ...
    const loc = utcOf(next);
    expect(loc.dayOfWeek).toBe(1); // Monday UTC
